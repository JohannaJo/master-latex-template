...
In the context of rule mining and KG embeddings the OWA however has a central problem; it fails to provide counterexamples. Because missing facts are assumed to be neither true nor false, there are no false facts to give as examples when mining rules or training an embedding model. In the context of the predictions, marked with red in \cref{KB_predictions} we want to maximize B over D. Under the OWA, however, there are no false facts, so we don't know what B and D are. Someone (TODO: who?) therefore introduced a different paradigm called the \textit{partial completeness assumption} (PCA).

The assumption is that for some entities $x$, $y$, if $r(x, y) \in KB true$ then
\[\forall y' : r(x, y') \in KBtrue \cup NEWtrue \Rightarrow r(x, y') \in KBtrue\]. 

Note the difference between PCA and PCWA.
