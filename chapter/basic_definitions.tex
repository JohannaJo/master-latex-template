...
Three different performance indexes are commonly used to evaluate the embedding quality of a model. As they are simple to calculate, they are suitable for measuring performance on a large scale. With $\mathcal{T}$ as the set of triples ranked by the model and $\mathcal{S}$ as the set of true triples, we can define three performance indexes.

\subsubsection{Hits@K}
Hits@K is the probability of finding the correct triple in the top K ranked triples. Often $k=10$ or lower values are used. This metric measures the model' ability to rank positive triples higher than corrupted ones.
