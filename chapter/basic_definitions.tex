...
The work of Ebisu et al. showed that PCA confidence may underestimate the likelihood of a predicted triple due to non-injective mappings of variables in rules. Consider for example the rule $hasParent(x, z) \wedge hasChild(z, y) \Rightarrow hasSibling(x, y)$, and the KG from example \ref{mini_KG_rules}. By mapping both $x$ and $y$ to the entity \texttt{Ann}, body of this rule can be instantiated with the triples \texttt{Ann, hasParent, Carol} and \texttt{(Carol, hasChild, Ann)}, resulting in the incorrect fact that Ann is her own sibling. GRPO confidence therefore excludes instances such as this one that don't map variables to unique entities when computing confidence.

In addition to this adjustment, the GPRO confidence is defined
In the work where this metric is proposed it is used to calculate the quality of a predicted fact, and uses GPRO confidence calculated on both the head and tail of the consequent triple, while PCA confidence originally only was calculated on the tail of the consequent triple. 



CONTINUE WITH DEFINING GPRO AND GRANK
