...
As discussed in Section \ref{TransE_peaked_in_2013} TransE is not able to learn certain relation qualities, and upon further inspection of the embedding vectors for relations in TransE the vector values all tend toward zero (see \ref{TransE_embedding_family} and \ref{TransE_embedding_WN18RR}). This can partially be explained by TransE's tendency to push the embeddings of symmetric relations toward the zero vector and it's lack of ability to effectively embed complex relations. Both issues explained in section \ref{KG_embeddings_section}. It seems that while scoring decently on the performance metrics during model selection, TransE has not properly embedded the relations in the KG. 
...
\begin{subfigure}{.5\textwidth}
  ...
  \label{venn_family}
\end{subfigure}
\caption[New rules grouped by which KGEs]{Distribution of new rules grouped by which KGE was used to created the extended KG from which the rule was mined. Duplicates are removed.}
\label{venn}
\end{figure}
...
By looking at figure \ref{rule_dist_models_hbar} it becomes apparent that there in fact fewer different rules mined from the RandomBaseline extensions. In the WN18RR case no new rules were found, and a single original rule was also never mined. The omitted rule,
